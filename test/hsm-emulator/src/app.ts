...
export function createApp(port: number) {
  ...
  app.post('/hsm/translate-pin', async function handler(ffReq, ffReply) {
    const requestBody = JSON.parse(JSON.stringify(ffReq.body))
    const { pinBlock, sourcePan, sourceFormat, targetPan, targetFormat } =
      requestBody
    ...
    logger.info(`Translated PIN block from ${sourceFormat} to ${targetFormat}`)
  ...
  app.post('/hsm/verify-pin', async function handler(ffReq, ffReply) {
    ...
    // Check if DUKPT parameters are provided
    const isDukpt = tr31IpekUnderLmk && ksnHex && lmkHex
    if (isDukpt) {
      logger.info(`Verifying PIN with DUKPT decryption using KSN: ${ksnHex}`)
    }
    ...
    const isValid = verifyPin(
      ...
      tr31IpekUnderLmk,
      ksnHex,
      lmkHex || AES_MERCHANT_ASE_LMK_HEX // Default to merchant LMK if not provided
    )
    ...
    ffReply.code(200).send({
      isValid,
      format,
      decryptionMethod: isDukpt ? 'DUKPT' : 'CLEAR'
    })
  })
