...
export function createPOSStore(deps_: StoreDependencies): PosStoreService {
  ...
  return {
    ...
    addPOS: (requestId: string, POSHost: string) => addPOS(deps, requestId, POSHost),
    deletePOS: (requestId: string) => deletePOS(deps, requestId),
  }
}

const getPOS = async (deps: StoreDependencies, requestId: string): Promise<string> => {
...
const addPOS = async (deps: StoreDependencies, requestId: string, POSHost: string) => {
  await deps.redis.set(requestId, POSHost);
  deps.logger.info(`POS ${POSHost} was added for requestId: ${requestId}`);
}
