...
Asset and Peer liquidity accounts are created when calling `createAsset` and `createPeer` [GraphQL Admin API mutations](../../api-reference/mutations.md) respectively, while liquidity accounts for payment pointers, incoming and outgoing payments are created on-the-fly during payment processing or web monetization events.
