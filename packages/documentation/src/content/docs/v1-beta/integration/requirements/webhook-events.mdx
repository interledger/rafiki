...
When an event occurs, the [`backend`](/v1-beta/integration/deployment/services/backend-service) service makes a <Badge text="POST" variant="success" size="medium" /> request to your configured webhook endpoint. The `backend` service expects a `200` status in return.
...
Additionally, the [Local Playground](/v1-beta/integration/playground/overview) contains example payloads in the <LinkOut href="https://github.com/interledger/rafiki/tree/main/bruno/collections/Rafiki/Sample%20Webhook%20Events">Bruno collection</LinkOut> you can use to test your webhook service integration.
...
<Badge text="Two-phase transfer" variant="tip" size="medium" /> An outgoing payment
for $12 is complete. $11.50 was sent. You choose to keep $0.50 as a service fee.
