...
Rafiki is open-source software maintained by a dedicated team and freely available to any licensed <Tooltip content="A regulated entity that provides and maintains payment accounts for its customers" client:load>[account servicing entity](/es/v1-beta/overview/concepts/account-servicing-entity)</Tooltip> (ASE) wanting to implement Interledger and [Open Payments](#pagos-abiertos) on users' accounts.
