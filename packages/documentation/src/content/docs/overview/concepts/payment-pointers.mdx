...
```http title="Example response"
{
  ...
  "assetCode": "USD",
  "assetScale": 2,
  "authServer": "https://auth.wallet.example.com/some-tenant-id",
  "resourceServer": "https://wallet.example.com/some-tenant-id",
}
```

In a multi-tenant environment, the `authServer` and `resourceServer` URLs include the `tenantId` to ensure that Open Payments requests are properly routed to the correct tenant.
