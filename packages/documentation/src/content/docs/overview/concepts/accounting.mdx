...
An incoming payment liquidity account represents the value received from a completed incoming payment. When an incoming payment is created via the <LinkOut href="https://openpayments.dev/apis/resource-server/operations/create-incoming-payment/">Open Payments APIs</LinkOut>, a corresponding liquidity account is automatically created. You will have one liquidity account per incoming payment.
