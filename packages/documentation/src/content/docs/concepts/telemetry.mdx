...
The configuration can be tested in our [Local Playground](/integration/deployment/playground/overview) by providing the environment variables in the table above to `happy-life-backend` in the <LinkOut href="https://github.com/interledger/rafiki/blob/main/localenv/happy-life-bank/docker-compose.yml">`docker-compose.yml`</LinkOut> file.
