...
A key registry is a list of keys stored by a client that requires access to Open Payments resources protected by an authorization server (AS). A key is generated and added to the registry by a client and the client is represented by a [wallet address](/reference/glossary#payment-pointer). The wallet address's associated key registry is exposed publically at the path `WALLET_ADDRESS/jwks.json` in anticipation of a grant request on an AS.
