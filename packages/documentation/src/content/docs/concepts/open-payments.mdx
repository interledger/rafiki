...
Rafiki’s [`backend`](/integration/deployment/servicesbackend-service) service is the main service for handling business logic and external communication. The service is responsible for, among other things, exposing the endpoints of the Open Payments APIs for clients to perform account management tasks. Every request and response is validated against the <LinkOut href="https://github.com/interledger/open-payments/tree/main/openapi">Open Payments specification</LinkOut>.
