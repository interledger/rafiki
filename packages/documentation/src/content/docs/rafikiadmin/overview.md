...
Rafiki Admin provides a user-friendly administrative user interface for interacting with the [Admin APIs](#admin-apis). In this web application, you'll be able to view and manage peering relationships, assets, and wallet addresses, among other settings. Rafiki Admin is a Remix application for querying info and executing mutations against the Rafiki [Admin APIs](#admin-apis).

In order to run Rafiki Admin you'll need a local Rafiki backend instance up and running, as well as a Kratos identity service and Mailslurper. All of these services, along with Rafiki Admin are available within the local playground for testing and devlopment purposes. See the local environment [README](../../localenv/README) for more information on how to set it up. TLDR: to get the whole environment up, run the command `pnpm localenv:compose up` from the root of the project.
