...
- Create the operator tenant record in the Postgres database with `OPERATOR_TENANT_ID` prior to startup (on startup, the backend syncs the operator's API secret from `ADMIN_API_SECRET`)
