...
The amounts within ILP packets are used to update account balances in your accounting database (TigerBeetle or Postgres). Amounts are adjusted based on Rafiki’s configured [exchange rates](/integration/before-you-begin#exchange-rates) and the asset used by the sending (outgoing) and receiving (destination) accounts. The total amount received for the STREAM connection is tracked in Redis in order to support <LinkOut href="https://interledger.org/developers/rfcs/stream-receipts/">STREAM receipts</LinkOut>.

## GraphQL Backend Admin API

The `backend` service exposes a GraphQL [Backend Admin API](/integration/deployment/apis/graphql/admin-api-overview#backend-admin-api) to manage assets, peers, wallet addresses, Open Payments resources, and several types of liquidity.
