...
The Rafiki `frontend` service provides an internal admin interface for managing your Rafiki instance. It communicates with the Backend Admin API to faciliate administrative tasks. See [Frontend service](/integration/deployment/services/frontend-service) for more information.
...
In Docker Compose, networks and volumes are necessarily for enabling communication between services and persisting data storage for containers.
