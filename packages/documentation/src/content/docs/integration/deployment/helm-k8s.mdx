...
We recommended use of the latest vendor supported version for each of the software dependencies listed in this section.
:::
...
Create a `values.yaml` file to customize your Rafiki deployment. Below is a template that you can use, but must be configured for your specific environment.
...
Save this file as `values.yaml` in your working directory.
...
## Configuring ingress with NGINX Ingress Controller
...
### 3. Configure Rafiki with ingress

Create or update your `values.yaml` file with the following ingress configuration:
...
ingress:
  ...
  # Auth Server ingress configuration
  authServer:
    enabled: true
    hosts:
...
To enable TLS for secure communications:

```yaml
global:
  tls:
    enabled: true
    # Use this if you're using cert-manager
    certManager:
      enabled: true
...
Configure resource limits for Rafiki components:
...
1. Check if PostgreSQL pods are running: `kubectl get pods -l app.kubernetes.io/name=postgresql`
2. Check PostgreSQL logs: `kubectl logs pod/rafiki-postgresql-0`
3. Verify that the database passwords match those in your values.yaml

### Tigerbeetle initialization failures

1.  Check Tigerbeetle logs: `kubectl logs pod/tigerbeetle-0`
2.  Ensure that the PVC for Tigerbeetle has been created correctly `kubectl get pvc -l app.kubernetes.io/name=tigerbeetle`
3.  Verify that the cluster ID is consistent across all components

### Ingress issues

1.  Verify NGINX Ingress Controller is running: `kubectl get pods -n ingress-nginx`
2.  Check if your DNS records are correctly pointing to the ingress controller's external IP
3.  Check the Ingress resource: `kubectl get ingress`
4.  Check Ingress controller logs: `kubectl logs -n ingress-nginx deploy/nginx-ingress-ingress-nginx-controller`
5.  Verify that TLS secrets exist if HTTPS is enabled: `kubectl get secrets`

### TLS certificate problems

1.  If using cert-manager, check if certificates are properly issued: `kubectl get certificates`
2.  Check certificate status: `kubectl describe certificate [certificate-name]`
3.  Check cert-manager logs: `kubectl logs -n cert-manager deploy/cert-manager`

### Service unavailable

1.  Check if services are running: `kubectl get services`
2.  Verify pod health: `kubectl describe pod [pod-name]`
3.  Check for resource constraints: `kubectl top pods`

### Connectivity between components

1.  Ensure all required services are running: `kubectl get services`
...
- **RBAC**: Use Kubernetes Role-Based Access Control to limit access to your cluster
- **Secrets Management**: Consider using a secrets management solution
- **Regular Updates**: Keep your Rafiki deployment updated
...
Note: The above example assumes you have a [Container Storage Interface (CSI)](https://kubernetes.io/docs/concepts/storage/volumes/#csi) driver capable of volume snapshots. Adjust the `volumeSnapshotClassName` according to your cluster setup.
...
tigerbeetle:
  replicaCount: 3 # Use multiple replicas for fault tolerance
  persistence:
    enabled: true
...
This configuration sets up multiple replicas for each Rafiki component and enables replication for both PostgreSQL, Redis, and Tigerbeetle.
