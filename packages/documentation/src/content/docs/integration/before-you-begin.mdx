...
There are three key components that you need to set up prior to integrating Rafiki.

1. A service to provide Rafiki with current [exchange rates](#exchange-rates)
2. A trusted [identity provider](#identity-providers) to handle user authentication
3. A mechanism for handling Rafiki [webhook events](#webhook-events)
