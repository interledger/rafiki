...
- `apiSecret`: The API secret used to secure requests made for this tenant. This secret is used to generate HTTP signatures, which are used to authenticate requests to the Backend Admin API.
...
- `WALLET_ADDRESS_URL`: The URL of the tenant's wallet address service. This setting is used to configure the base URL for wallet addresses created for the tenant.
