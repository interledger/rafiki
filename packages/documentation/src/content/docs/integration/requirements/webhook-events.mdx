...
The main communication channel between you and your Rafiki instance is composed of the Backend Admin API and a set of webhook events. The operator can choose to receive webhook events for all tenants by setting the `SEND_TENANT_WEBHOOKS_TO_OPERATOR` environment variable to `true`.
