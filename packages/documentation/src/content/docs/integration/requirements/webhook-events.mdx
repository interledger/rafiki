...
<details>
  ...
  <Mermaid
    ...
    alt Accept partial payment
      ...
      R-->>ASE: success: true
      ASE->>ASE: Apply partial payment<br />to associated incoming payment
...
| `DB_ENCRYPTION_SECRET`                      | `backend` | A base64-encoded 32-byte secret used to encrypt/decrypt transmitted payment data (`dataToTransmit`) stored on incoming/outgoing payment records and events.                                                                                                                      |
