...
:::note
Rafiki doesn't hold _user_ account balances. Instead, Rafiki keeps track of the liquidity in your instance's asset, peer, and payment accounts in its own database.
:::
...
<details>
  ...
  <Mermaid
    graph={`sequenceDiagram
      ...
      R->>FSP: Fires incoming_payment.created event to webhook endpoint
      FSP->>FSP: No action required
...
<details>
  ...
  <Badge text='Single-phase transfer' variant='tip' size='medium' /> An incoming payment of $10 was completed.
  <Mermaid
    graph={`sequenceDiagram
      participant R as Rafiki
      participant FSP as Financial service provider

      R->>FSP: Fires incoming_payment.completed event to webhook endpoint,<br/>receivedAmount: $10
...
/>
  ...
  <Badge text='Two-phase transfer' variant='tip' size='medium' /> An incoming payment of $10 was completed.
  <Mermaid
    graph={`sequenceDiagram
      participant R as Rafiki
      participant FSP as Financial service provider
...
#### Incoming payment partial payment received
...
<details>
  ...
  <Mermaid
    ...
    R->>FSP: Fires incoming_payment.partial_payment_received event to webhook endpoint
    FSP->>FSP: Reviews event data
    alt Accept partial payment
      FSP->>R: Backend Admin API call: ConfirmPartialIncomingPayment
      R-->>FSP: success: true
      FSP->>FSP: Process payment (e.g. credit receiver)
...
| ------------------------------------------- | --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ENABLE_PARTIAL_PAYMENT_DECISION`           | `backend` | Enables an FSP to act upon (approve/reject) a partial payment. The default is `false`.                                                                                                                                                                                           |
...
| `PARTIAL_PAYMENT_DECISION_MAX_WAIT_MS`      | `backend` | The maximum time, in milliseconds, Rafiki will wait for an FSP to confirm or reject a partial incoming payment. Rafiki will reject the payment if a response isn't received in time. Only used if this value is less than the `PARTIAL_PAYMENT_DECISION_SAFETY_MARGIN_MS` value. |
...
\$2.55 was received before the payment expired. The recipient is thus credited with \$2.55.
  ...
  <Mermaid
    graph={`sequenceDiagram
      ...
      R->>FSP: Fires incoming_payment.expired event to webhook endpoint,<br/>receivedAmount: $2.55
      FSP->>R: Backend Admin API call: createIncomingPaymentWithdrawal
      R-->>FSP: success: true
...
An outgoing payment for \$12 was created.
  ...
  <Mermaid
    graph={`sequenceDiagram
      ...
      R->>FSP: Fires outgoing_payment.created event to webhook endpoint,<br/> debitAmount: $12
      ...
      alt Account has sufficient funds
        ...
        FSP->>R: Backend Admin API call: cancelOutgoingPayment,<br/>Reason: insufficient funds
        R-->>FSP: success: true
      end
...
<details>
  ...
  <Badge text='Single-phase transfer' variant='tip' size='medium' /> An outgoing payment for \$12 is complete. \$11.50 was sent. You choose to keep \$0.50 as a service fee.
  <Mermaid
    graph={`sequenceDiagram
      participant R as Rafiki
      participant FSP as Financial service provider

      R->>FSP: Fires outgoing_payment.completed event to webhook endpoint,<br/>debitAmount: $12, sentAmount: $11.50
      FSP->>R: Backend Admin API call: createOutgoingPaymentWithdrawal
      R-->>FSP: success: true
      FSP->>FSP: Remove hold and deduct $12 from sender's account,<br/>credit your account with $0.50
...
An outgoing payment for \$12 failed. \$8 was sent successfully.
  ...
  <Mermaid
    graph={`sequenceDiagram
      ...
      R->>FSP: Fires outgoing_payment.failed event to webhook endpoint,<br/>debitAmount: $12, sentAmount: $8
      FSP->>R: Backend Admin API call: createOutgoingPaymentWithdrawal
      R-->>FSP: success: true
...
The wallet address, `https://wallet.example.com/carla_garcia` was requested but doesn't exist.
  ...
  <Mermaid
    graph={`sequenceDiagram
      ...
      R->>FSP: Fires wallet_address.not_found event to webhook endpoint,<br/>wallet address: https://wallet.example.com/carla_garcia
      FSP->>R: Backend Admin API call: createWalletAddress,<br/>url: https://wallet.example.com/carla_garcia,<br/>public name: Carla Eva Garcia
      R-->>FSP: success: true
...
A wallet address received a Web Monetization payment of \$0.33
  ...
  <Mermaid
    graph={`sequenceDiagram
      ...
      R->>FSP: Fires wallet_address.web_monetization event to webhook endpoint,<br/>receivedAmount: $0.33
...
Your asset liquidity for USD (asset scale: 2) drops below \$100.00.
  ...
  <Mermaid
    graph={`sequenceDiagram
      ...
      R->>FSP: Fires asset.liquidity_low event to webhook endpoint,<br/>asset: USD (scale: 2, id: "abc")
...
The liquidity for your peer, Happy Life Bank, drops below \$100.00 USD.
  ...
  <Mermaid
    graph={`sequenceDiagram
      ...
      R->>FSP: Fires peer.liquidity_low event to webhook endpoint,<br/>peer: Happy Life Bank (asset: "USD", scale: 2, id: "abc")
