...
The main communication channel between you and your Rafiki instance is composed of the Backend Admin API and a set of webhook events. In a multi-tenant environment, the operator can choose to receive webhook events for all tenants by setting the `SEND_TENANT_WEBHOOKS_TO_OPERATOR` environment variable to `true`.
...
In a multi-tenant environment, the webhook URL is configured on a per-tenant basis. The operator can configure a tenant-specific webhook URL using the Backend Admin API. If a tenant-specific webhook URL is not configured, Rafiki will use the default `WEBHOOK_URL` environment variable.
