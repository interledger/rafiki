...
## Expose an exchange rate endpoint <Badge text="Required" variant="danger" />
...
## Peers <Badge text="Conditionally Required" variant="caution" />

While you could use Rafiki soley for transfers between accounts on your ledger, you must add one or more peers if you intend to enable Interledger payments on your accounts. A peer is another ASE that you want to connect with via Interledger and is likely running their own Rafiki instance.

[Set up peers in your Rafiki instance](/integration/requirements/peers)

## Integrate with an identity provider (IdP) <Badge text="Conditionally Required" variant="caution" />
