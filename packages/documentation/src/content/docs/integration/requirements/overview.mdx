...
- [ ] You have generated a UUID v4 for `OPERATOR_TENANT_ID` and a strong, random `API_SECRET`
- [ ] You are HMAC-signing Backend Admin API requests (HMAC SHA-256) and include `tenant-id` and `signature` headers (or you're using the Rafiki Admin app, which handles this automatically)
