...
The header's purpose is to secure communications between the IdP and authorization servers. Its value should be a shared secret known to both entities. When the IdP server sends requests to the authorization server, the IdP must provide the secret via this header.
