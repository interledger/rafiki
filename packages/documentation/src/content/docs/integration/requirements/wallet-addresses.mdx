...
- Tenants can only create wallet addresses for their own tenant
...
The `tenantId` is determined from the HTTP signature. All requests to create a wallet address must be signed using the HTTP signature method. The request must include a `tenantId` header, and the `Signature-Input` header must include the `tenantId` header in its list of signed components.
:::
