...
- [In response to the `WALLET_ADDRESS_NOT_FOUND` webhook event](#create-wallet-addresses-in-response-to-a-webhook-event)
...
The [`WALLET_ADDRESS_NOT_FOUND`](/integration/requirements/webhook-events#wallet-address-not-found) event fires when a wallet address is requested through the Open Payments Get Wallet Address API, but Rafiki can't find the address.
