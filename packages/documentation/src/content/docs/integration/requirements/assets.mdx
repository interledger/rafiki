...
<Tabs>
  ...
  :::note[Tenant ID and HTTP signatures]
    The `tenantId` is determined from the HTTP signature. All requests to create an asset must be signed using the HTTP signature method. The request must include a `tenantId` header, and the `Signature-Input` header must include the `tenantId` header in its list of signed components.
  :::
