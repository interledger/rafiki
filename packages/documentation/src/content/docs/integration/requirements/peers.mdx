...
<Tabs>
  ...
  :::note[Tenant ID and HTTP signatures]
    In a multi-tenant environment, the `tenantId` is determined from the HTTP signature. All requests to create a peer must be signed using the HTTP signature method. The `tenantId` from the signature is used to verify the identity of the requester. The request must include a `tenantId` header, and the `Signature-Input` header must include the `tenantId` header in its list of signed components.
  :::
  ...
  :::note[Tenant ID in the peer response]
    In a multi-tenant environment, the `peer` object in the response will include the `tenantId` of the tenant to which the peer belongs. This `tenantId` is used to identify the tenant when processing requests related to the peer.
  :::
...
<Tabs>
  ...
  :::note[Tenant ID and HTTP signatures]
    In a multi-tenant environment, the `tenantId` is determined from the HTTP signature. All requests to update a peer must be signed using the HTTP signature method. The `tenantId` from the signature is used to verify the identity of the requester. The request must include a `tenantId` header, and the `Signature-Input` header must include the `tenantId` header in its list of signed components.
  :::
