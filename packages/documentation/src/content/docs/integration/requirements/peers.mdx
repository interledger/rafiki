...
<Tabs>
  ...
  :::note[Tenant ID and HTTP signatures]
    The `tenantId` is determined from the HTTP signature. All requests to create a peer must be signed using the HTTP signature method. The `tenantId` from the signature is used to verify the identity of the requester. The request must include a `tenantId` header, and the `Signature-Input` header must include the `tenantId` header in its list of signed components.
  :::
...
<Tabs>
  ...
  :::note[Tenant ID and HTTP signatures]
    The `tenantId` is determined from the HTTP signature. All requests to update a peer must be signed using the HTTP signature method. The `tenantId` from the signature is used to verify the identity of the requester. The request must include a `tenantId` header, and the `Signature-Input` header must include the `tenantId` header in its list of signed components.
  :::
