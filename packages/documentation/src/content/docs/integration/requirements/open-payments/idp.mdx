...
The following variables must be configured for the `auth` service. Operators configure the `IDENTITY_SERVER_URL` and `IDENTITY_SERVER_SECRET` variables on a per-tenant basis. These values are stored in the `auth` service's tenant table and are used to secure communications between the `auth` service and the tenant's IdP.
