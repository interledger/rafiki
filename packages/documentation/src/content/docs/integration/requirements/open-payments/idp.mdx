...
In a multi-tenant environment, each tenant uses their own IdP.
...
The following variables must be configured for the `auth` service. In a multi-tenant environment, the `IDENTITY_SERVER_URL` and `IDENTITY_SERVER_SECRET` variables are configured on a per-tenant basis. These values are stored in the `auth` service's tenant table and are used to secure communications between the `auth` service and the tenant's IdP.
