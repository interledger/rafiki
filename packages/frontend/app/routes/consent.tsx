...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  if (urlConsentChallenge && !sessionConsentChallenge) {
    return setChallengeAndRedirect({
      session,
      location: '.',
      challengeName: 'consent_challenge',
      challenge: urlConsentChallenge
    })
  }

  const HYDRA_ADMIN_URL = 'http://hydra:4445'
  const hydraUrl = `${HYDRA_ADMIN_URL}/admin/oauth2/auth/requests/consent?consent_challenge=${sessionConsentChallenge}`

  try {
    const hydraGetResponse = await axios.get(hydraUrl)
    if (hydraGetResponse.status !== 200) {
      throw new Error(
        `Hydra responded with status: ${hydraGetResponse.status}: ${hydraGetResponse.statusText}`
      )
    }
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  try {
    const hydraPutResponse = await axios.put(
      `http://hydra:4445/admin/oauth2/auth/requests/consent/accept?consent_challenge=${sessionConsentChallenge}`,
      {
        // other data Hydra needs
