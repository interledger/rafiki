...
export const loader = async ({ request, params }: LoaderArgs) => {
    ...
    if (fees?.pageInfo.hasPreviousPage) {
      previousPageUrl = `/assets/${assetId}/view-fees?before=${fees.pageInfo.startCursor}`
    }
  
    if (fees?.pageInfo.hasNextPage) {
      nextPageUrl = `/assets/${assetId}/view-fees?after=${fees.pageInfo.endCursor}`
    }
  
    return json({ fees, previousPageUrl, nextPageUrl })
  }

export default function AssetFeesPage() {
    const { fees, previousPageUrl, nextPageUrl } =
