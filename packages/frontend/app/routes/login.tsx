...
export const loader = async ({ request }: LoaderArgs) => {
  ...
  if (urlLoginChallenge && !sessionLoginChallenge) {
    ...
    const hydraUrl = `http://localhost:4445/oauth2/auth/requests/login?login_challenge=${sessionLoginChallenge}`
...
export async function action({ request }: ActionArgs) {
  ...
  const response = await axios.put(
    `http://localhost:4445/oauth2/auth/requests/login/accept?login_challenge=${loginChallenge}`,
    {
      subject: username
