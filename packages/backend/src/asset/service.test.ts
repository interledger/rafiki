  ...
  describe('Create or Get Asset', (): void => {
    ...
    describe('create', (): void => {
      ...
      test('Cannot create duplicate asset', async (): Promise<void> => {
        ...
        await expect(assetService.create(options)).resolves.toMatchObject(
          options
        )
        ...
        ({ withdrawalThreshold }): void => {
          ...
          beforeEach(
            async (): Promise<void> => {
              ...
              assert.ok(!isAssetError(asset))
              await expect(asset.withdrawalThreshold).toEqual(
                withdrawalThreshold
              )
              assetId = asset.id
            }
          ...
          `(
            ...
            async ({ withdrawalThreshold }): Promise<void> => {
              ...
              await expect(assetService.getById(assetId)).resolves.toEqual(
                asset
              )
            }
          )
