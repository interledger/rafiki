...
export class MaxPacketAmountController implements StreamController {
  ...
  isProbeComplete(): boolean {
    ...
    const verifiedLargeCapacity =
      ...
      this.verifiedPathCapacity.isGreaterThanOrEqualTo(
        RateProbe.MAX_PROBE_AMOUNT
      )
  ...
  getMaxPacketAmountLimit(): PositiveInt {
    return this.state.type === MaxPacketState.UnknownMax
      ? Int.MAX_U64
  ...
  applyRequest({ sourceAmount }: StreamRequest) {
    return (reply: StreamReply): PaymentError | void => {
      if (
        reply.isReject() &&
        reply.ilpReject.code === IlpError.F08_AMOUNT_TOO_LARGE
      ) {
  ...
  private reduceMaxPacketAmount(
    ...
    try {
      ...
      log.debug(
        'handling F08. remote received: %s, remote max: %s',
        remoteReceived,
        remoteMaximum
      )
      ...
      log.debug(
        'handling F08 without metadata. source amount: %s',
        sourceAmount
      )
    ...
    if (this.state.type === MaxPacketState.UnknownMax) {
      ...
      log.debug(
        'reducing max packet amount from %s to %s',
        this.state.maxPacketAmount,
        newMax
      )
    } else {
  ...
  private adjustPathCapacity(log: Logger, ackAmount: Int) {
    ...
    if (
      ...
      log.debug(
        'discovered precise max packet amount: %s',
        this.state.maxPacketAmount
      )
      this.state = {
        type: MaxPacketState.PreciseMax,
