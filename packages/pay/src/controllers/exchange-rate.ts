...
export class ExchangeRateController implements StreamController {
  ...
  applyRequest({
    sourceAmount,
    log
  }: StreamRequest): (reply: StreamReply) => void {
    return ({ destinationAmount }: StreamReply) => {
      ...
      const packetUpperBoundRate = Ratio.of(
        destinationAmount.add(Int.ONE),
        sourceAmount
      )
  ...
  estimateDestinationAmount(sourceAmount: Int): [Int, Int] {
    const lowEndDestination = sourceAmount
      .multiplyFloor(this.lowerBoundRate)
