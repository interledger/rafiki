  ...
  return async (request: StreamRequest): Promise<StreamReply> => {
    ...
    if (isFulfill(ilpReply)) {
      ...
      log.debug(
        'got %s Reject: %s',
        ilpReply.code,
        ILP_ERROR_CODES[ilpReply.code as IlpErrorCode]
      )

      if (ilpReply.message.length > 0 || ilpReply.triggeredBy.length > 0) {
        log.trace(
          'Reject message="%s" triggeredBy=%s',
          ilpReply.message,
          ilpReply.triggeredBy
        )
      }
    } else {
    ...
    if (streamReply) {
      if (streamReply.sequence.notEquals(sequence)) {
        log.error(
          'discarding STREAM reply: received invalid sequence %s',
          streamReply.sequence
        )
      } else if (
        +streamReply.ilpPacketType === IlpPacketType.Reject &&
        isFulfill(ilpReply)
      ) {
