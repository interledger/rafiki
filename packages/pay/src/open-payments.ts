...
const queryIncomingPayment = async (
  ...
  return fetchJson(url, OPEN_PAYMENT_QUERY_ACCEPT_HEADER)
    ...
    .catch((err) =>
      log.debug('destinationPayment query failed: %s', err?.message)
    )
...
export const queryAccount = async (
  ...
  const accountUrl =
    AccountUrl.fromPaymentPointer(destinationAccount) ??
    AccountUrl.fromUrl(destinationAccount)
  if (!accountUrl) {
    log.debug(
      'payment pointer or account url is invalid: %s',
      destinationAccount
    )
...
const isNonNullObject = (o: any): o is Record<string, any> =>
...
const validateOpenPaymentsCredentials = async (
  o: any
): Promise<PaymentDestination | undefined> => {
  if (!isNonNullObject(o)) {
    return
  }
  ...
  if (typeof ilpStreamConnection === 'string') {
    details = await fetchJson(
      ilpStreamConnection,
      OPEN_PAYMENT_QUERY_ACCEPT_HEADER
    )
  } else {
    details = ilpStreamConnection
  }
  const { ilpAddress: destinationAddress, sharedSecret: sharedSecretBase64 } =
    details
  ...
  return {
    destinationAsset: {
      code: destinationAmount.assetCode,
      scale: destinationAmount.assetScale
    },
    destinationAddress,
    sharedSecret
  }
...
const validateConnectionCredentials = (
  o: any
...
const validateOpenPaymentsAmount = (
  o: Record<string, any>
