...
describe('interledger.rs integration', () => {
  ...
  it('pays to SPSP server', async () => {
    ...
    rustNodeContainer = await new GenericContainer(
      'interledgerrs/ilp-node:latest'
    )
      .withEnvironment({
    ...
    const scope = nock('https://mywallet.com')
      ...
      .delay(1000)
      .reply(200, () =>
        Axios.get(`http://${host}/accounts/receiver/spsp`).then(
          (res) => res.data
        )
      )
...
describe('interledger4j integration', () => {
  ...
  it('pays to SPSP server', async () => {
    ...
    connectorContainer = await new GenericContainer(
      'interledger4j/java-ilpv4-connector:0.5.1'
    )
      .withEnvironment({
        // Hostname of Redis container
        'redis.host': 'redis_4j',
        'interledger.connector.adminPassword': adminPassword,
        'interledger.connector.spsp.serverSecret':
          randomBytes(32).toString('base64'),
        'interledger.connector.enabledFeatures.localSpspFulfillmentEnabled':
          'true',
