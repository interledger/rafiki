...
describe('account urls', () => {
  it('AccountUrl#fromPaymentPointer', () => {
    ...
    expect(
      AccountUrl.fromPaymentPointer('$user:pass@example.com')
    ).toBeUndefined()
    expect(AccountUrl.fromPaymentPointer('$localhost:3000')).toBeUndefined()
    expect(
      AccountUrl.fromPaymentPointer('$example.com?foo=bar')
    ).toBeUndefined()
    expect(AccountUrl.fromPaymentPointer('$example.com#hash')).toBeUndefined()

    expect(
      AccountUrl.fromPaymentPointer('$example.com/alice')!.toString()
  ...
  it('AccountUrl#fromUrl', () => {
    ...
    expect(
      AccountUrl.fromUrl('https://user:pass@wallet.example')
    ).toBeUndefined()
    expect(AccountUrl.fromUrl('https://wallet.example:8080/')).toBeUndefined()

    expect(
      AccountUrl.fromUrl('https://wallet.example/account?foo=bar')!.toString()
    ).toBe('https://wallet.example/account?foo=bar')
  })

  it('AccountUrl#toEndpointUrl', () => {
    expect(
      AccountUrl.fromPaymentPointer('$cool.wallet.co')!.toEndpointUrl()
    ).toBe('https://cool.wallet.co/.well-known/pay')
    expect(
      AccountUrl.fromUrl(
        'https://user.example?someId=123#bleh'
      )!.toEndpointUrl()
    ).toBe('https://user.example/.well-known/pay?someId=123#bleh')
  ...
  it('AccountUrl#toBaseUrl', () => {
    ...
    expect(
      AccountUrl.fromUrl('https://user.example?someId=123#bleh')!.toBaseUrl()
    ).toBe('https://user.example/.well-known/pay')
  ...
  it('AccountUrl#toString', () => {
    ...
    ).toBe('https://wallet.example/user/account?baz#bleh')
  })

  it('AccountUrl#toPaymentPointer', () => {
    expect(
      AccountUrl.fromUrl('https://somewebsite.co/')!.toPaymentPointer()
    ).toBe('$somewebsite.co')
    expect(
      AccountUrl.fromUrl('https://user.example?someId=123')!.toPaymentPointer()
    ).toBeUndefined()
    expect(
      AccountUrl.fromUrl('https://example.com/bob/#hash')!.toPaymentPointer()
    ).toBeUndefined()
    expect(
      AccountUrl.fromUrl('http://somewebsite.co/')!.toPaymentPointer()
    ).toBeUndefined()

    expect(
      AccountUrl.fromPaymentPointer('$example.com/')!.toPaymentPointer()
    ).toBe('$example.com')
    expect(
      AccountUrl.fromPaymentPointer('$example.com/charlie/')!.toPaymentPointer()
    ).toBe('$example.com/charlie')
    expect(
      AccountUrl.fromPaymentPointer('$example.com/charlie')!.toPaymentPointer()
    ).toBe('$example.com/charlie')
  })
...
describe('integer operations', () => {
  ...
  it('Ratio#from', () => {
    ...
    expect(Ratio.from(12.34)).toEqual(
      Ratio.of(Int.from(1234)!, Int.from(100) as PositiveInt)
    )
  ...
  it('Ratio#reciprocal', () => {
    expect(Ratio.of(Int.ONE, Int.TWO).reciprocal()).toEqual(
      Ratio.of(Int.TWO, Int.ONE)
    )
    expect(Ratio.of(Int.TWO, Int.ONE).reciprocal()).toEqual(
      Ratio.of(Int.ONE, Int.TWO)
    )
  ...
  it('Ratio#isEqualTo', () => {
    expect(
      Ratio.of(Int.from(8)!, Int.TWO).isEqualTo(Ratio.of(Int.from(4)!, Int.ONE))
    ).toBe(true)
    expect(
      Ratio.of(Int.from(0)!, Int.TWO).isEqualTo(Ratio.of(Int.from(4)!, Int.ONE))
    ).toBe(false)
  })
  ...
  it('Ratio#toString', () => {
    ...
    expect(Ratio.of(Int.ONE, Int.from(3) as PositiveInt).toString()).toBe(
      (1 / 3).toString()
    )
  })
  ...
  it('Ratio#toJSON', () => {
    ...
    expect(
      Ratio.of(Int.from(821)!, Int.from(1200) as PositiveInt).toJSON()
    ).toEqual(['821', '1200'])
  })
})
