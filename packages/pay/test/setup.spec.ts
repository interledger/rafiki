...
const setupNock = (options: setupNockOptions) => {
  ...
  const incomingPaymentId =
    options.incomingPaymentId !== null
      ? options.incomingPaymentId || uuid()
      : undefined
  const accountUrl =
    options.accountUrl !== null
      ? options.accountUrl || 'https://wallet.example/alice'
      : undefined
  const destinationPayment =
    options.destinationPayment !== null
      ? options.destinationPayment ||
        `${accountUrl}/incoming-payments/${incomingPaymentId}`
      : undefined
  const completed =
    options.completed !== null ? options.completed || false : undefined
  ...
  const expiresAt =
    options.expiresAt !== null
      ...
      : undefined
  const description =
    options.description !== null ? options.description || 'Coffee' : undefined
  const externalRef =
    options.externalRef !== null ? options.externalRef || '#123' : undefined
...
describe('open payments', () => {
  ...
  it('quotes an Incoming Payment', async () => {
    ...
    const { minDeliveryAmount, minExchangeRate, paymentType } =
      await startQuote({
        plugin,
        destination,
        prices,
  ...
  it('fails if Incoming Payment url is not HTTPS or HTTP', async () => {
    await expect(
      fetchPaymentDetails({
        destinationPayment: 'oops://this-is-a-wallet.co/incoming-payment/123'
      })
  ...
  it('fails if given a payment pointer as an Incoming Payment url', async () => {
    await expect(
      fetchPaymentDetails({ destinationPayment: '$foo.money' })
    ).resolves.toBe(PaymentError.QueryFailed)
  })
  ...
  `(
    'resolves and validates an Incoming Payment with $description',
    async ({ connectionId }) => {
      const {
        accountUrl,
        destinationPayment,
      ...
      await expect(
        fetchPaymentDetails({ destinationPayment })
  ...
  it('fails if account query fails', async () => {
    const scope = nock(accountUrl.origin)
      ...
      .reply(500)
    await expect(
      fetchPaymentDetails({ destinationAccount: account.id })
    ).resolves.toBe(PaymentError.QueryFailed)
    scope.done()
  })
  ...
  it('fails if connection query response is invalid', async () => {
    ...
    const scope2 = nock('https://wallet.com')
      .get(`/${connectionId}`)
      .reply(200, '3')
    await expect(
      fetchPaymentDetails({
  ...
  it('resolves credentials from SPSP', async () => {
    ...
    const credentials = await fetchPaymentDetails({
      destinationAccount: '$alice.mywallet.com'
    })
    expect(credentials).toMatchObject({
      sharedSecret,
      destinationAddress,
  ...
  it('fails if SPSP query fails', async () => {
    ...
    await expect(
      fetchPaymentDetails({ destinationAccount: '$open.mywallet.com' })
    ).resolves.toBe(PaymentError.QueryFailed)
    scope.done()
  })
  ...
  it('fails if SPSP query response is invalid', async () => {
    ...
    await expect(
      fetchPaymentDetails({ destinationAccount: '$alice.mywallet.com' })
    ).resolves.toBe(PaymentError.QueryFailed)
    scope2.done()
    ...
    const scope3 = nock('https://wallet.example')
      ...
      .reply(200, '3')
    await expect(
      fetchPaymentDetails({ destinationAccount: '$wallet.example' })
    ).resolves.toBe(PaymentError.QueryFailed)
    scope3.done()
  })
  ...
  it('follows SPSP redirect', async () => {
    ...
    const credentials = await fetchPaymentDetails({
      destinationAccount: '$wallet1.example'
    })
    expect(credentials).toMatchObject({
      sharedSecret,
      destinationAddress
  ...
  it('fails on SPSP redirect to non-HTTPS endpoint', async () => {
    ...
    await expect(
      fetchPaymentDetails({ destinationAccount: '$wallet1.example' })
    ).resolves.toBe(PaymentError.QueryFailed)
  ...
  it('fails if the payment pointer is semantically invalid', async () => {
    await expect(
      fetchPaymentDetails({ destinationAccount: 'ht$tps://example.com' })
    ).resolves.toBe(PaymentError.InvalidPaymentPointer)
  })

  it('fails if query part is included', async () => {
    await expect(
      fetchPaymentDetails({ destinationAccount: '$foo.co?id=12345678' })
    ).resolves.toBe(PaymentError.InvalidPaymentPointer)
  })
  ...
  it('validates given STREAM credentials', async () => {
    ...
    await expect(
      fetchPaymentDetails({ sharedSecret, destinationAddress })
    ).resolves.toMatchObject({
      sharedSecret,
      destinationAddress
    })
  ...
  it('fails if no mechanism to fetch STREAM credentials was provided', async () => {
    await expect(fetchPaymentDetails({})).resolves.toBe(
...
describe('setup flow', () => {
  ...
  it('fails if SPSP response is invalid', async () => {
    const scope = nock('https://example4.com')
      .get('/foo')
      .reply(200, { meh: 'why?' })
...
describe('quoting flow', () => {
  ...
  it('fails if the probed rate is 0', async () => {
    ...
    const plugin = createPlugin(
      ...
      createRateMiddleware(
        new RateBackend(sourceAsset, destinationAsset, prices)
      ),
      streamReceiver
    )
    ...
    const plugin = createPlugin(
      ...
      async (prepare, next) => {
        ...
        if (+prepare.amount > maxPacketAmount) {
          ...
          largestAmountReceived = Math.max(
