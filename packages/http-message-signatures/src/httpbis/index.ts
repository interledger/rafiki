...
export function parseSignatures(
  ...
  signatureDictionary.forEach(([signature], signatureName) => {
    ...
    if (Array.isArray(signature) || !isByteSequence(signature)) {
      throw new Error(
        `Error parsing signature value for signature '${signatureName}'. Expected a Byte Sequence.`
      )
    }
    if (!signatureInputDictionary.has(signatureName)) {
      throw new Error(
        `Error parsing signature '${signatureName}'. No corresponding signature input.`
      )
    }
    signatures.set(signatureName, Buffer.from(signature.base64Value, 'base64'))
  })

  signatureInputDictionary.forEach(
    ([components, parameters], signatureName) => {
      const value = signatures.get(signatureName)
      if (!value) {
        throw new Error(
          `Error parsing signature input for '${signatureName}'. No corresponding signature.`
        )
      }
      // Expect components to be an Inner List + parameters which is returned from the parser as Item[]
      if (!Array.isArray(components)) {
        throw new Error(
          `Error parsing signature input for signature '${signatureName}'. Expected an Inner List.`
        )
      }
      ...
      const dateValue = (value: BareItem, key: string): Date => {
        if (typeof value !== 'number') {
          throw new Error(
            `Error parsing signature input parameter '${key}'. Expected an integer but got '${value}' with typeof ${typeof value}.`
          )
        }
        const val = new Date(value * 1000)
        if (!val || val.toString() === 'Invalid Date') {
          throw new Error(
            `Error converting signature input parameter '${key}' to a timestamp from '${value}'.`
          )
        }
        return val
      }
      const algorithmValue = (value: BareItem, key: string): Algorithm => {
        ...
        throw new Error(
          `Error signature input parameter '${key}'. '${value}' is not a known algorithm.`
        )
      }
      ...
      returnValue.set(signatureName, {
        ...
        components: components.map(([component, params], i) => {
          ...
          if (typeof component !== 'string') {
            throw new Error(
              `Error parsing component at index '${i}'. Expected a string but got a ${typeof component}.`
            )
          }
          return component
        }),
...
export async function verify(
  ...
  signatures.forEach(
    ({ value, components, signatureParams, keyid, alg }, signatureName) => {
      if (!keyid) {
        return false
      }
      ...
      const data = Buffer.from(
        buildSignedData(request, components || [], signatureParams)
      )
  ...
  return (
    await Promise.all(
      Object.entries(signatures).map(
        ([signatureName, { components, parameters, raw }]) => {}
      )
    )
  ).every((result) => result)
}
