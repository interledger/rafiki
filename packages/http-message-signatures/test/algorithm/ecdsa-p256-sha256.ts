...
describe('ecdsa-p256-sha256', () => {
    ...
    describe('signing', () => {
      it('signs a payload', async () => {
        ...
        expect(sig).to.satisfy((arg: Buffer) =>
          verify('sha256', Buffer.from(data), ecdsaKeyPair.publicKey, arg)
        )
      })
    })
    describe('verifying', () => {
  ...
  describe('specification examples', () => {
    ...
    before('load rsa key', async () => {
      ecKeyPem = (
        await promisify(readFile)(join(__dirname, '../etc/ecdsa-p256.pem'))
      ).toString()
    })
    describe('response signing', () => {
      ...
      it('successfully signs a payload', async () => {
        ...
        expect(sig).to.satisfy((arg: Buffer) =>
          verify('sha256', Buffer.from(data), ecKeyPem, arg)
        )
      })
      ...
      it.skip('successfully verifies a signature', async () => {
        const sig = Buffer.from(
          'n8RKXkj0iseWDmC6PNSQ1GX2R9650v+lhbb6rTGoSrSSx18zmn6fPOtBx48/WffYLO0n1RHHf9scvNGAgGq52Q==',
          'base64'
        )
        expect(sig).to.satisfy((arg: Buffer) =>
          verify('sha256', Buffer.from(data), ecKeyPem, arg)
        ...
        expect(
          await createVerifier('ecdsa-p256-sha256', ecKeyPem)(data, sig)
        ).to.equal(true)
      })
    })
  })
