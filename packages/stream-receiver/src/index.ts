...
export class StreamServer {
  ...
  public getStreamData(prepare: IlpPrepare): Buffer | undefined {
    const streamPacket = this.decryptPacket(prepare)
    if (!streamPacket) {
      return
    }

    const dataFrame = streamPacket.frames.find(
      (frame): frame is StreamDataFrame => frame.type === FrameType.StreamData
    )

    return dataFrame?.data
  }

  private decryptPacket(prepare: IlpPrepare) {
    ...
    return Packet._deserializeUnencrypted(decrypt(encryptionKey, prepare.data))
  }
